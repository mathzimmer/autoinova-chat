...
async function searchVehiclesForAI(filters: {
  ...
  if (!filters.category) {
    // Regra documentada no prompt: sem categoria explícita, busca SÓ carros.
    const tipoLower = (filters.vehicleType || "").toLowerCase();
    const MOTO_TYPES = ["naked", "esportiva", "street", "trail", "custom", "scooter", "cub"];
    if (!MOTO_TYPES.includes(tipoLower)) {
      allVehicles = allVehicles.filter(v => (v.category || "").toLowerCase() === "carros");
      console.log(`[StockSync] Category default "carros" → ${allVehicles.length} results`);
    }
  }
  if (filters.fuel) {
