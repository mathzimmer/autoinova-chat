...
import {
  ...
  listWNConversations,
  listWNMessages,
  updateWNConversation,
  markWNConversationRead,
  getWNConversationById,
  sendTextFromNumber,
  sendMediaFromNumber,
  upsertWNConversation,
  saveWNMessage,
} from "./whatsappMultiNumber";
...
// ── WhatsApp Multi-Number Router ─────────────────────────────────────────────
const whatsappNumbersRouter = router({
  // Admin: list all registered numbers
  list: adminProcedure.query(async () => {
    return listWhatsappNumbers();
  }),

  // Admin: get single number
  getById: adminProcedure.input((v: any) => v as { id: number }).query(async ({ input }) => {
    return getWhatsappNumberById(input.id);
  }),

  // Admin: create new number
  create: adminProcedure
    .input((v: any) => v as {
  ...
  update: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const { id, ...data } = input;
      return updateWhatsappNumber(id, data);
    }),

  // Admin: delete number
  delete: adminProcedure
    .input((v: any) => v as { id: number })
    .mutation(async ({ input }) => {
      await deleteWhatsappNumber(input.id);
      return { success: true };
    }),

  // Protected: list conversations for a number (admin sees all, user sees assigned)
  listConversations: protectedProcedure
    .input((v: any) => v as { whatsappNumberId: number; limit?: number })
    .query(async ({ input }) => {
      return listWNConversations(input.whatsappNumberId, input.limit || 50);
    }),

  // Protected: list messages for a conversation
  listMessages: protectedProcedure
    .input((v: any) => v as { conversationId: number; limit?: number })
    .query(async ({ input }) => {
      return listWNMessages(input.conversationId, input.limit || 100);
    }),

  // Protected: update conversation (name, status, notes, etc.)
  updateConversation: protectedProcedure
    .input((v: any) => v as {
      id: number;
      contactName?: string;
    ...
    .mutation(async ({ input }) => {
      const { id, ...data } = input;
      await updateWNConversation(id, data);
      return { success: true };
    }),

  // Protected: mark conversation as read
  markRead: protectedProcedure
    .input((v: any) => v as { conversationId: number })
    .mutation(async ({ input }) => {
      await markWNConversationRead(input.conversationId);
      return { success: true };
    }),

  // Protected: send text message
  sendText: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const result = await sendTextFromNumber(input.phoneNumberId, input.to, input.text);
      ...
      await saveWNMessage({
        ...
        messageType: "text",
        direction: "outbound",
        senderName: ctx.user?.name || "Agente",
        status: "sent",
        timestamp: Date.now(),
      });

      // Update conversation preview
      await updateWNConversation(input.conversationId, {});
      const conv = await getWNConversationById(input.conversationId);
      if (conv) {
        await upsertWNConversation({
          whatsappNumberId: input.whatsappNumberId,
  ...
  sendMedia: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const result = await sendMediaFromNumber(
        input.phoneNumberId,
        input.to,
        input.mediaUrl,
        input.mediaType,
      ...
      if (!result.success) throw new Error(result.error || "Failed to send media");
